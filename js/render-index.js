...
(function(){
  async function init(){
    ...
    if(heroEl && data.hero){
      heroEl.innerHTML = `<div><h1 class="section-title">${data.hero.title}</h1><p class="muted"><strong>${data.hero.subtitle}</strong></p><div data-social-buttons="social-buttons.html"></div><div class="badges" style="margin-top:12px"></div></div><div class="profile"><img src="${data.hero.profile}" alt="Profile photo"/></div>`;
      const badgesWrap = heroEl.querySelector('.badges');
      (data.hero.badges||[]).forEach(b=>{ const sp=document.createElement('span'); sp.className='badge'; sp.textContent=b; badgesWrap.appendChild(sp); });
    }
    ...
    if(ec && (Array.isArray(data.education) || Array.isArray(data.certifications))){
      ...
      (data.education||[]).forEach(e=>{ const item=document.createElement('div'); item.className='edu-item'; item.innerHTML = `<img src="${e.logo}" alt="${e.school} logo"/><div><div><strong>${e.school}</strong></div><div class="muted">${e.detail}</div></div>`; lcont.appendChild(item); });
      ...
      (data.certifications||[]).forEach(c=>{ const item=document.createElement('div'); item.className='cert-item'; item.innerHTML = `<img src="${c.logo}" alt="${c.name} logo"/><div><div><strong>${c.name}</strong></div><div class="muted">${c.when}</div></div>`; rcont.appendChild(item); });
